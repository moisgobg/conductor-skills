...
| `index.md` (top-level) | **Section 3.0** | "Resuming setup: Scaffolding is complete. Transitioning to completion and handoff." |
...
1.  **Copy Initial Workflow:**
    -   Copy `assets/workflow.md` to `conductor/workflow.md`.
2.  **Determine Mode:** Use the `ask_user` tool to let the user choose their preferred workflow.
    - **questions:**
        - **header:** "Workflow"
...
2.  **Save Conductor Files:** Add and commit all files with the commit message `conductor(setup): Initialize project context and standards`.

---

## 3.0 COMPLETION AND HANDOFF
Once the `conductor/` directory is created, the index is generated, and files are committed, announce that setup is complete.

1.  **Summary:** Present a final summary of the initialized scaffolding (e.g., tech stack selected, workflow chosen, styles copied).
2.  **Proactive Suggestion:** Ask the user if they would like to start defining their first actionable track (feature or bug fix) right now.
3.  **Internal Handoff:**
    - If the user agrees, you MUST use the `conductor-new-track` skill to begin planning immediately.
    - If the user declines, inform them they can start a track later by using the `conductor-new-track` skill directly.
